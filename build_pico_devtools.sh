...
# make pico_tools directory
mkdir -p ${basedir}/pico_tools

#tell pkg_config we have a special directory to consider for builds here..
#need for openocd build..
export PKG_CONFIG_PATH="${PKG_CONFIG_PATH}:${basedir}/pico_tools/lib/pkgconfig"

# Pull pico_sdk
echo "** Pulling RP Pico SDK **"
git clone https://github.com/raspberrypi/pico-sdk.git pico_sdk
cd pico_sdk
...
cmake ../
make -j16
cp ./_deps/picotool/picotool ${basedir}/pico_tools/bin
cp ./_deps/picotool-src/udev/99-picotool.rules ${basedir}
cd ../../
...
cd jimtcl_build
./configure --prefix=${basedir}/pico_tools
make -j32
make install
...
cd openocd_build
./bootstrap
./configure --enable-cmsis-dap --prefix=${basedir}/pico_tools
make -j32
make install
...
export PICOTOOL_FETCH_FROM_GIT_PATH=/home/kb3gtn/sandbox/rp_pico_projects/pico_tools/bin
EOF
...
echo "** UDEV Rules **"
echo " install 99-picotool.rules /etc/udev/rules.d then udevadm control --reload-rules "
